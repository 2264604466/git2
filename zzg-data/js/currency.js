    ...
    $("nav ul li a").click(function () {
        ...
        if (dls.length == 0) {
            ...
            htmlobj = $.ajax({
                ...
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    alert('当前页面未完成');
                    $('#content').html('<p style="color:red;font-size:30px">当前页面未完成</p>');
                    // // 状态码
                    // console.log(XMLHttpRequest.status);
                    // // 状态
                    // console.log(XMLHttpRequest.readyState);
...
function SDD() {
    ...
    $('nav').css({ width: '220px' })
    cite.show();
    $('#content').css({ 'left': '220px', width: 'calc(100% - 220px)' })
...
function HDD() {
    ...
    cite.hide();
    $('#content').css({ 'left': '60px', width: 'calc(100% - 60px)' })
...
$("nav ul li a[name]").css({ "color": "red" })